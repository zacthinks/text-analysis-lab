...
class _Project:
    ...
    def operation_for_artifact(self, artifact):
        ...
        opid = next(
            (op for op, aid in self.operations.items() if aid == artifact.artifact_id),
            None,
        )
        return (
            None
            if opid is None
            else {"operation_id": opid, "operator_id": f"operator_{opid}"}
        )
...
def _record(project, opid, source, output, operator):
    ...
    project.sources[opid] = [
        {"source_label": "source", "source_artifact_id": source.artifact_id}
    ]
    project.outputs[opid] = [
        {"output_label": "output", "artifact_id": output.artifact_id}
    ]
...
def test_transform_texts_like_replays_frozen_count_tfidf_pipeline(
    tmp_path: Path,
) -> None:
    ...
    tfidf_matrix = _Artifact(
        project, "tfidf", ArtifactType.SPARSE_MATRIX, ["alpha", "beta", "gamma"]
    )
...
def test_column_normalization_is_not_replayable_for_single_new_text(
    ...
    count_matrix = _Artifact(
        project, "count", ArtifactType.SPARSE_MATRIX, ["alpha", "beta"]
    )
    _record(project, "countop", table, count_matrix, count)
    normalizer = MatrixNormalizer(axis="columns")
    norm_matrix = _Artifact(
...
def test_transform_texts_like_replays_frozen_feature_trim_mask(tmp_path: Path) -> None:
    ...
    trimmed = _Artifact(
        project, "trimmed", ArtifactType.SPARSE_MATRIX, ["alpha", "gamma"]
    )
