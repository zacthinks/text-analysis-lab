...
class MatrixNormalizer(BaseTranslator):
    ...
    def output_specs(
        ...
        if source.artifact_type.value not in {"sparse_matrix", "dense_matrix"}:
            raise OperatorError(
                "MatrixNormalizer requires a sparse_matrix or dense_matrix source."
            )
        return OutputSpec(
    ...
    def validate_operation_params(
        ...
        *,
        ...
        if params:
            raise OperatorError(
                f"MatrixNormalizer does not accept operation parameters; got {sorted(params)}."
            )
        return {}
        ...
        *,
        ...
        if self._source_type == "sparse_matrix":
            values = (
                ...
                else sparse.csr_matrix(normalized)
            )
        else:
            values = (
                normalized.toarray()
                if sparse.issparse(normalized)
                else np.asarray(normalized)
            )
        return BatchResult(
            outputs={
    ...
    def make_translate_worker(
        ...
        if mode != "translate" or self.axis != "rows":
            raise OperatorError(
                "Parallel MatrixNormalizer workers are available only for row-wise normalization."
            )
    ...
    def from_json_state(cls, state: Mapping[str, Any]) -> MatrixNormalizer:
        return cls(
            axis=str(state.get("axis", "rows")), norm=str(state.get("norm", "l2"))
        )
    ...
    def load_intermediate_state(
        ...
        *,
        ...
        if not isinstance(state, Mapping):
            raise OperatorError(
                "MatrixNormalizer intermediate state must be a mapping."
            )
